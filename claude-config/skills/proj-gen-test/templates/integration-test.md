...
class UserServiceTest {
    ...
    @Test
    @Sql("/sql/init-data.sql")  // 测试前执行
    @Sql(scripts = "/sql/clean-data.sql", executionPhase = AFTER_TEST_METHOD)  // 测试后执行
    void getUserById_Exists_ReturnsUser() {
        // 使用 init-data.sql 准备的数据
    }
}
```
...
2. **使用 H2 内存数据库** - 测试环境使用 H2，无需安装 MySQL
3. **H2 兼容模式** - `MODE=MySQL` 使 H2 兼容 MySQL 语法
4. **使用 test profile** - 隔离测试环境配置
5. **测试真实场景** - 包括成功和失败路径
6. **测试顺序** - 集成测试之间互不依赖，可任意顺序执行
7. **性能考虑** - 集成测试较慢，不要在集成测试中测试能被 Mock 的场景
